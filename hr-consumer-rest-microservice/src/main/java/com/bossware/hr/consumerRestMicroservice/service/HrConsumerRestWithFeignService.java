package com.bossware.hr.consumerRestMicroservice.service;

import org.springframework.boot.autoconfigure.condition.ConditionalOnProperty;
import org.springframework.scheduling.annotation.Scheduled;
import org.springframework.stereotype.Service;

import com.bossware.hr.consumerRestMicroservice.client.HrConsumerRestServiceClient;
import com.bossware.hr.consumerRestMicroservice.dto.EmployeeRequest;

@Service
@ConditionalOnProperty(name = "loadBalancing" , havingValue = "feing")
public class HrConsumerRestWithFeignService {

	private final HrConsumerRestServiceClient hrConsumerRestService;

	public HrConsumerRestWithFeignService(HrConsumerRestServiceClient hrConsumerRestService) {
		
		this.hrConsumerRestService = hrConsumerRestService;
	}
	
	@Scheduled(fixedRate = 3_000)
	public void callRestApi() {	
		var request = new EmployeeRequest();
		request.setIdentity("38394916980");
		request.setFirstName("kate");
		request.setLastName("austen");
		request.setIban("TR670006251748489918423875");
		request.setSalary(200_000.0);
		request.setCurrency("USD");
		request.setDepartment("IT");
		request.setBirthYear(1988);
		request.setJobStyle("PART_TIME");
		request.setPhoto("");
		//System.out.println(hrConsumerRestService.hire(request));
		
		
		System.out.println(hrConsumerRestService.getEmployeeByIdentity("38394916980").toString());
		
		//System.out.println(hrConsumerRestService.fire("38394916980"));
	}
	
	
	
}
