...
@Service
@ConditionalOnProperty(name = "loadBalancing" , havingValue = "standard")
public class HrConsumerRestService {

	private final DiscoveryClient discoveryClient;
	private List<ServiceInstance> instances;
	private AtomicInteger counter = new AtomicInteger();
	public HrConsumerRestService(DiscoveryClient discoveryClient) {
	...
	public void callSynchronousRestApi() {	
		...
		var response = restTemplate.getForEntity("http://%s:%d/hr/api/v1/employees/%s".formatted(instance.getHost(),instance.getPort(),"38394916980"), String.class);
